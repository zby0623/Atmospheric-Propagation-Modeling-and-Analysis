...
5. simulation_scale_length_simulation parameter_type of PSD: simulation results of ellipticity and eccentricity with varying inner scale for a specific type of PSD [NOTE: The results obtained using the Tatarskii and mvK PSDs are very similar; therefore, their corresponding panels in Fig. 7 appear nearly identical. However, these raw data reveal subtle differences. They also suggest that the difference in prediction accuracy between the Tatarskii and mvK PSDs is mainly caused by the different Cn2 alues derived from each model.]
