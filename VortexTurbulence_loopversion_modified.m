%% Initialize MATLAB, allocating matrices and set up parameters
clc
clear all
% clear variables
close all
...
incrementNumber = turb_nscr;
incrementNumber_prop = prop_nscr;
frameNumber=500; % how many frames to be obtained in one simulation
...
[X,Y] = meshgrid(-wmax/2:delta:wmax/2,-wmax/2:delta:wmax/2); % mesh the field 
...
for i=1:3
    plot(cn2,SI(i,:),'o','MarkerSize',10,'MarkerFaceColor',colors(i,:),'MarkerEdgeColor',colors(i,:));hold on;
end
hold off;
legend('m=0','m=1','m=2','Location','Best');
xlabel('C_n^2 (m^{-2/3})');
